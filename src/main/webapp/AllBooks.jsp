...
div.sticky {
  ...
  opacity: .90;
    ...
    html { 
  background: url(Backgrounds/bg4.jpg) no-repeat center center fixed; 
...
<!--    navigation bar ends   -->
    ...
    String newBook= (String)session.getAttribute("oldBook");
    if(newBook!=null && newBook.equals("updated")){%><h3 align="center"><%=msg
    %></h3>
        <%
            session.removeAttribute("oldBook");
}%>
 <%  msg="Deleted Successfully!";
    newBook= (String)session.getAttribute("deleteBook");
    if(newBook!=null && newBook.equals("deleted")){%><h3 align="center"><%=msg
    %></h3>
        <%
            session.removeAttribute("deleteBook");
}%>
 
            <div class="row" style="width: auto"><font style="font-size: 18px">&emsp;&emsp;All Books</font><hr></div>
        ...
        <tr>
            <td><br>
                <a href="bookPage_1?bId=<%=rs.getInt("book_id") %>"><img class="sideByside" src="image/<%=rs.getString("book_id")%>.jpg"></a><br>
            ...
            <td><a href="bookPage_1?bId=<%=rs.getInt("book_id") %>"><b><%=rs.getString("book_name") %></b></a><br>
                <br> By <%=rs.getString("author_name") %><br><br>
            Released on <%=rs.getDate("release_date") %></td>
             <td><%if(admin){%>
                <a href="deleteBook?bId=<%=rs.getInt("book_id") %>">
                        <input type="button" value="delete" onclick="return confirm ('Are you sure want to delete?');" style="background: none; border: thin solid #ccc; color: #3E1B0F; font-size: 14px"></a><br>
                       <%}%> 
            
        <br><br> <a href="updateBookInfo.jsp?upId=<%=rs.getInt("book_id") %>">
            <%if(admin){%>
            <input type="button" value="update" style="background: none; border: thin solid #ccc; color: #3E1B0F; font-size: 14px"></a>
            <%}%> 
            <br>
    ...
    <%}%>
            ...
            <!--    erase "height:700px" to set the height according to its elements   -->
        </div></div>
